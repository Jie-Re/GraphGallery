...
clean_changes = []
attacked_changes = []
print(f"size of splits.test_nodes {np.size(splits.test_nodes)}")
...
for target in splits.test_nodes:
    ...
    # eps = 1
    # TODO: eps_range
    eps_range = list(np.arange(1, 5.02, 0.02))
    ...
    for eps in eps_range:
        ...
        if np.argmax(clean_predict) != np.argmax(clean_defended_predict):
            print(
                f"!WARNING: predict label for clean graph has been changed from {np.argmax(clean_predict)} to "
                f"{np.argmax(clean_defended_predict)}(true label is {target_label})")
        if np.argmax(attacked_predict) != np.argmax(attacked_defended_predict):
            print(
                f"!WARNING: predict label for attacked graph has been changed from {np.argmax(attacked_predict)} to "
                f"{np.argmax(attacked_defended_predict)}(true label is {target_label})")

    clean_changes.append(clean_change)
    attacked_changes.append(attacked_change)

################### TODO: Plot #########################
x = np.size(splits.test_nodes)
plt.figure()
plt.xlabel('target nodes')
plt.ylabel('change after defense')
