...
# eps-target-loop, write the result out to the file
# row is target
# col is eps
with open("eps(0.01,5.01)_target_change.txt", "w") as f:
    # clean_change = []
    # attacked_change = []
    for target in splits.test_nodes:
        ...
        # eps is 0 will cause divide-0 error
        for eps in list(np.arange(0.01, 5.01, 0.01)):
            # clean_defended_g
            ...
            f.write(f"{clean_predict[clean_label] - clean_defended_predict[clean_defended_label]},"
                    f"{attacked_predict[attacked_label] - attacked_defended_predict[attacked_defended_label]}\t")
